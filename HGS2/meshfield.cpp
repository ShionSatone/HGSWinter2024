...
void CollisionMeshField(void)
{
	...
	BlackSanta* pBlackSanta = GetBlackSanta();
	...
	{//地面の数
		...
		{//使っている透明でない壁
			...
			{
				Fieldvec = aPos[1] - aPos[0];//壁のベクトル
				Posvec = pBlackSanta->pos - aPos[0];//壁に対するプレイヤーのベクトル
				PosOldvec = pBlackSanta->posOld - aPos[0];//壁に対するプレイヤーの旧ベクトル
				movevec = pBlackSanta->pos - pBlackSanta->posOld;//プレイヤーの移動ベクトル
				if ((Fieldvec.x * Posvec.y) - (Fieldvec.y * Posvec.x) <= 0.0f && (Fieldvec.x * PosOldvec.y) - (Fieldvec.y * PosOldvec.x) >= 0.0f && pBlackSanta->pos.z < g_aMeshField[nCntMeshField].pos.z + MESHFIELD_WIDTH * 0.5f && pBlackSanta->pos.z > g_aMeshField[nCntMeshField].pos.z - MESHFIELD_WIDTH * 0.5f)
				{//地面の下
					FieldCross = (Fieldvec.x * movevec.y) - (Fieldvec.y * movevec.x);
					PosCross = (Posvec.x * movevec.y) - (Posvec.y * movevec.x);
					PosCross /= FieldCross;
					...
					{
						Hit = aPos[0] + Fieldvec * PosCross;
						movevec = pBlackSanta->pos - Hit;//プレイヤーの移動ベクトル
						Norvec = D3DXVECTOR3(Fieldvec.y, -Fieldvec.x, Fieldvec.z);
						D3DXVec3Normalize(&Norvec, &Norvec);
						Dovec = Norvec * ((-movevec.x * Norvec.x) + (-movevec.y * Norvec.y));
						pBlackSanta->pos += Dovec * 1.001f;
						pBlackSanta->move.y = 0.0f;
						if (pBlackSanta->motionType == MOTIONTYPE_JUMP)
						{
							pBlackSanta->motionType = MOTIONTYPE_LANDING;
						}
					}
				}
			}
