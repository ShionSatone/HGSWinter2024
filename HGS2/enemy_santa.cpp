...
void InitPlayer(void)
{
	...
	g_Player.nLife = SANTA_LIFE;
	g_Player.state = SANTASTATE_NORMAL;
...
void UpdatePlayer(void)
{
	if (g_Player.bUse)
	{
		...
		//移動量の更新(減衰)
		g_Player.move.x += (SPEED_DEF - g_Player.move.x) * SANTA_INA;
		g_Player.move.z += (SPEED_DEF - g_Player.move.z) * SANTA_INA;

		g_Player.pStage = NULL;

		CollisionSnowBall(g_Player.pos, SANTA_SIZE);
		...
		case SANTASTATE_NORMAL:
			if (g_Player.nLife <= 0)
			{
				g_Player.state = SANTASTATE_DIE;
			}
			SetPositionShadow(g_Player.nIdxShadow, g_Player.pos, g_Player.scale, 200.0f);
			SetLife(g_Player.pos + g_Player.aModel[0].pos + g_Player.aModel[1].pos + g_Player.aModel[10].pos, (float)((float)g_Player.nLife / (float)SANTA_LIFE), g_Player.nIdxLife);
			break;
		case SANTASTATE_DIE:
			...
			if (nCnt >= 20)
			{
				...
				g_Player.state = SANTASTATE_APPEAR;
			}
			break;
		}
	}
